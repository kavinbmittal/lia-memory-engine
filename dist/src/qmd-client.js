...
export class QMDClient {
    ...
    constructor(opts) {
        ...
        if (!isAbsolute(opts.memoryDir)) {
            throw new Error(`[lia-memory-engine] QMDClient memoryDir must be an absolute path, got: "${opts.memoryDir}"`);
        }
        this.host = opts.host;
        this.port = opts.port;
    ...
    async startDaemon() {
        ...
        const spawnResult = await new Promise((resolve) => {
            ...
            child.on("error", (err) => {
                if (!settled) {
                    settled = true;
                    if (err.code === "ENOENT") {
                        this.logger.warn("[lia-memory-engine] QMD CLI not found. Install with: npm install -g @tobilu/qmd");
                    }
                    else {
        ...
        this.logger.warn("[lia-memory-engine] QMD daemon did not become healthy within 10 seconds — " +
    ...
    async ensureCollection() {
        try {
            ...
            this.logger.info(`[lia-memory-engine] QMD collection "${this.collectionName}" registered at ${this.memoryDir}`);
        }
        catch (err) {
            // Collection likely already exists, or qmd not installed — both are fine
            const message = err instanceof Error ? err.message : String(err);
            this.logger.warn(`[lia-memory-engine] QMD ensureCollection warning (non-fatal): ${message}`);
        }
    }
    /**
    ...
    async search(query, opts) {
        ...
        if (!response.ok) {
            throw new Error(`[lia-memory-engine] QMD HTTP error: ${response.status} ${response.statusText}`);
        }
        const data = (await response.json());
        if (data.error) {
            throw new Error(`[lia-memory-engine] QMD JSON-RPC error ${data.error.code}: ${data.error.message}`);
        }
    ...
    async searchCLI(query) {
        try {
            const { stdout } = await execFileAsync("qmd", ["search", query, "-c", this.collectionName, "-n", "5"], { timeout: 3000 });
            return stdout ?? "";
        }
        catch {
