...
import { rankDocuments, searchMemory, searchForContext, formatSearchResults, } from "../src/search.js";
// ── rankDocuments (BM25) ────────────────────────────────────────────────────
...
describe("searchMemory", () => {
    ...
    it("should find matches in memory files", async () => {
        ...
        await writeFile(join(tempDir, "memory", "preferences.md"), "User prefers dark mode and compact layout.\nFavorite color is blue.\n");
        await writeFile(join(tempDir, "memory", "daily", "2026-03-15.md"), "## 14:30\n\n**User:** What's the weather?\n\n**Agent:** It's sunny.\n\n---\n");
    ...
    it("should filter by date when days parameter is set", async () => {
        ...
        await writeFile(join(tempDir, "memory", "daily", `${oldDateStr}.md`), "## 10:00\n\n**User:** Old conversation about deployment\n\n---\n");
        await writeFile(join(tempDir, "memory", "daily", `${recentDateStr}.md`), "## 10:00\n\n**User:** Recent conversation about deployment\n\n---\n");
        // Search with 7-day filter — should only find recent
        const results = await searchMemory(tempDir, "deployment", 7);
        const files = results.map((r) => r.file);
        assert.ok(!files.some((f) => f.includes(oldDateStr)), "should not include files older than 7 days");
    });
    it("should not filter non-daily files by date", async () => {
        await writeFile(join(tempDir, "memory", "preferences.md"), "User prefers compact deployment layout.\n");
    ...
    it("should reject non-absolute paths", async () => {
        await assert.rejects(() => searchMemory("relative/path", "test"), (err) => {
...
describe("searchForContext", () => {
    ...
    it("should return formatted context when results exist", async () => {
        ...
        await writeFile(join(tempDir, "memory", "daily", `${yesterdayStr}.md`), "## 14:00\n\n**User:** How does the billing system work?\n\n**Agent:** It uses Stripe.\n\n---\n");
    ...
    it("should exclude today's transcript from results", async () => {
        ...
        await writeFile(join(tempDir, "memory", "daily", `${today}.md`), "## 10:00\n\n**User:** Today's billing question\n\n**Agent:** Answer.\n\n---\n");
    ...
    it("should return non-daily memory file results", async () => {
        await writeFile(join(tempDir, "memory", "preferences.md"), "User prefers dark mode for the billing dashboard.\n");
