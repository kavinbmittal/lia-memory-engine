...
import type { SearchResult, SearchMatch, BM25Doc, RankedDocument } from "./types.js";
...
export function rankDocuments(
  ...
  const BM25 = (BM25Import as any).default ?? BM25Import;
...
export async function searchMemory(
  ...
  if (topLevelFiles.length === 0 && dailyFiles.length === 0) return [];
  ...
  for (const subDir of subDirs) {
    ...
    if (!subPath.startsWith(memoryDir)) continue;
    let subFiles: string[] = [];
    try {
