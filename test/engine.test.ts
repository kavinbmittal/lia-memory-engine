...
import type { AgentMessage, LiaConfig, LiaDependencies, ContentBlock } from "../src/types.js";
...
function buildDeps(
  workspaceDir: string,
  overrides?: Partial<LiaDependencies>
): LiaDependencies {
  return {
...
function buildConfig(overrides?: Partial<LiaConfig>): LiaConfig {
...
describe("ingest", () => {
  ...
  it("should write transcript without storing message", async () => {
    ...
    const msg: AgentMessage = { role: "user", content: "Hello there" };
  ...
  it("should not write transcript when disabled", async () => {
    ...
    await assert.rejects(
      ...
      "should not create transcript when disabled"
    );
  });
});
...
describe("assemble", () => {
  ...
  it("should include auto-retrieval context when memory exists", async () => {
    ...
    const engine = new LiaContextEngine(
      buildConfig({ autoRetrieval: true, autoRetrievalTimeoutMs: 5000 }),
      buildDeps(tempDir)
    );
    ...
    if (result.systemPromptAddition) {
      assert.ok(
        result.systemPromptAddition.includes("memory"),
        "systemPromptAddition should reference memory"
      );
    }
  ...
  it("should not include auto-retrieval when disabled", async () => {
    const engine = new LiaContextEngine(
      buildConfig({ autoRetrieval: false }),
      buildDeps(tempDir)
    );
    await engine.bootstrap({ sessionId: "assemble-3" });

    const messages: AgentMessage[] = [{ role: "user", content: "test query" }];
...
describe("compact", () => {
  ...
  it("should compact messages passed by OpenClaw", async () => {
    ...
    assert.ok(
      (result.messages[0].content as string).includes("[Conversation context"),
      "first message should be the summary"
    );
  });
  ...
  it("should reset flush counter after compaction", async () => {
    ...
    const transcriptWrites: AgentMessage[][] = [];
  ...
  it("should prevent double compaction", async () => {
    ...
    const slowCompleteFn = async (): Promise<string> => {
      completionCount++;
    ...
    const engine = new LiaContextEngine(
      buildConfig(),
      buildDeps(tempDir, { completeFn: slowCompleteFn })
    );
  ...
  it("should handle completeFn failure gracefully", async () => {
    ...
    const engine = new LiaContextEngine(
      buildConfig(),
      buildDeps(tempDir, { completeFn: failingCompleteFn })
    );
...
describe("afterTurn", () => {
  ...
  it("should signal compaction when over threshold", async () => {
    const engine = new LiaContextEngine(
      buildConfig({ compactionThreshold: 0.8 }),
      buildDeps(tempDir)
    );
...
describe("findLastUserMessage behavior", () => {
  ...
  it("should find text in plain string user message", async () => {
    const { writeFile } = await import("node:fs/promises");
    await writeFile(
      join(tempDir, "memory", "test-topic.md"),
      "Information about unique-keyword-abc.\n"
    );
    ...
    if (result.systemPromptAddition) {
      assert.ok(
        result.systemPromptAddition.includes("unique-keyword-abc") ||
        result.systemPromptAddition.includes("memory"),
        "should use last user message for auto-retrieval search"
      );
    }
  });
  ...
  it("should skip tool-result-only user messages and find previous text message", async () => {
    const { writeFile } = await import("node:fs/promises");
    await writeFile(
      join(tempDir, "memory", "special-topic.md"),
      "Information about special-search-term.\n"
    );
...
describe("restart scenario", () => {
  ...
  it("should correctly flush after restart without duplicating", async () => {
    ...
    const moreMessages = [
      ...messages,
      { role: "user" as const, content: "Brand new message A" },
      { role: "assistant" as const, content: "Brand new message B" },
    ];
    await engine.afterTurn({
      sessionId: "restart-2",
