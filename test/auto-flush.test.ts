...
describe("extractContentText", () => {
  ...
  it("should extract text from text blocks", () => {
    const block: ContentBlock = { type: "text", text: "Hello from text block" };
  ...
  it("should format tool_use blocks with tool name", () => {
    const block: ContentBlock = { type: "tool_use", name: "memory_search", input: { query: "test" } };
  ...
  it("should format tool_result blocks with string content", () => {
    const block: ContentBlock = { type: "tool_result", content: "Search found 3 results" };
  ...
  it("should format tool_result blocks with array content", () => {
    const block: ContentBlock = {
      type: "tool_result",
      content: [{ type: "text", text: "result" }] as unknown as string,
    };
  ...
  it("should truncate long tool_result content to 200 chars", () => {
    ...
    const block: ContentBlock = { type: "tool_result", content: longContent };
    const result = extractContentText(block);
  ...
  it("should return empty string for unknown block types", () => {
    const block: ContentBlock = { type: "image", source: { data: "..." } };
    assert.strictEqual(extractContentText(block), "");
  });
...
describe("formatTranscript", () => {
  ...
  it("should skip empty content messages", () => {
    const messages: AgentMessage[] = [
      ...
      { role: "assistant", content: [{ type: "tool_use", name: "search", input: {} }] as ContentBlock[] },
