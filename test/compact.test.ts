...
describe("chunkMessages", () => {
  ...
  function buildLargeMessages(count: number, charsPerMsg: number): AgentMessage[] {
    const msgs: AgentMessage[] = [];
    for (let i = 0; i < count; i++) {
      msgs.push({
        role: i % 2 === 0 ? "user" : "assistant",
  ...
  it("should split at user message boundaries", async () => {
    ...
    for (let i = 0; i < chunks.length; i++) {
      assert.strictEqual(
        chunks[i][0].role,
        "user",
        `chunk ${i} should start with a user message`
      );
    }
  });
  ...
  it("should keep user-assistant turn pairs together", async () => {
    ...
    for (let i = 0; i < chunks.length - 1; i++) {
      ...
      assert.strictEqual(
        lastMsg.role,
        "assistant",
        `chunk ${i} should end with an assistant message (turn pair intact)`
      );
    }
  });
...
describe("multi-chunk compactMessages", () => {
  it("should call completeFn multiple times for large conversations", async () => {
    let callCount = 0;
    const countingCompleteFn = async (
      _model: string,
      _systemPrompt: string,
      _userContent: string
    ): Promise<string> => {
  ...
  it("should include part N of M in chunk prompts", async () => {
    ...
    const capturingCompleteFn = async (
      _model: string,
      systemPrompt: string,
      _userContent: string
    ): Promise<string> => {
    ...
    for (let i = 0; i < prompts.length; i++) {
      assert.ok(
        prompts[i].includes(`part ${i + 1} of ${prompts.length}`),
        `prompt ${i} should contain "part ${i + 1} of ${prompts.length}"`
      );
    }
  });
  ...
  it("should combine chunk summaries with part markers", async () => {
    let callCount = 0;
    const multiCompleteFn = async (): Promise<string> => {
      callCount++;
    ...
    const summaryContent = result.compactedMessages[0].content as string;
  ...
  it("should fail entirely if any chunk fails", async () => {
    let callCount = 0;
    const failOnSecondCompleteFn = async (): Promise<string> => {
      callCount++;
    ...
    await assert.rejects(
      () => compactMessages(msgs, failOnSecondCompleteFn, "test-model"),
      (err: Error) => {
  ...
  it("should produce identical output for single-chunk conversations", async () => {
    const mockFn = async (): Promise<string> => "Single summary";
