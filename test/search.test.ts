...
import type { QMDClient } from "../src/qmd-client.js";
...
function makeMockClient(overrides: Partial<{
  search: QMDClient["search"];
  searchCLI: QMDClient["searchCLI"];
}>  = {}): QMDClient {
  return {
  ...
  it("should respect timeout and return empty when search is slow", async () => {
    const client = makeMockClient({
      search: async (_query, opts) => {
        ...
        await new Promise<void>((resolve) => setTimeout(resolve, (opts.timeoutMs ?? 5000) + 500));
        return "late result";
      },
    });
...
describe("searchMemory", () => {
  it("should use full mode (n=10, full=true) when daemon is running", async () => {
    let capturedOpts: { n?: number; full?: boolean; timeoutMs?: number } = {};
    const client = makeMockClient({
      search: async (_query, opts) => {
        capturedOpts = opts;
        return "## Full results\n- Item 1\n- Item 2";
...
describe("formatSearchResults", () => {
  it("should format results with the query in the header", () => {
    const formatted = formatSearchResults(
      "## Top matches\n- Billing system uses Stripe\n- Invoices generated weekly",
      "billing system"
    );

    assert.ok(
      formatted.includes('Memory search: "billing system"'),
      "should include query in header"
    );
    assert.ok(
      formatted.includes("Billing system uses Stripe"),
      "should include the result content"
    );
  });
